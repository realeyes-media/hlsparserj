...
class MediaPlaylistTest {
    ...
    private var keyPlaylistIS: InputStream? = null
    private var keyPlaylist: MediaPlaylist? = null
...
//                .getResource("/mediaplaylist.m3u8").readText()
        keyPlaylistIS = MediaPlaylistTest::class.java
                .getResourceAsStream("/keyplaylist.m3u8")
        try {
//            mediaPlaylist = PlaylistFactory.parsePlaylist(
//                PlaylistVersion.TWELVE, mediaPlaylistIS
//            ) as MediaPlaylist?
            keyPlaylist = PlaylistFactory.parsePlaylist(
                    PlaylistVersion.TWELVE, keyPlaylistIS) as MediaPlaylist?
        } catch (e: Exception) {
            e.printStackTrace()
            Assert.fail()
        }
...
//    }
    ...
    @Test
    fun key() {
        val key: List<Key?>? = keyPlaylist?.segments?.get(0)?.keys
